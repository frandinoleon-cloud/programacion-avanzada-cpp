...
class RegistroDeVuelo {
    ...
    public:
        RegistroDeVuelo(int nuevaCapacidad) {
            ...
            alturas = std::make_unique<double[]> (capacidad);//double[capacidad];
...
int main() {
    ...
    std::cout << "Cuenta antes de momnitorear: " <<registroSolo.getAltura(0) <<std:: endl;

    std::cout <<"Cuenta antes de monitorear: "<<registroCompartido.use_count() <<std::endl;
